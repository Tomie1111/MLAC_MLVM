...
// =================================================================
// 2. Incremental Workload Function (Updated for P99)
// =================================================================
async function runIncrementalWorkload(scenarioName, targetFunction) {
    ...
    for (const currentStep of STEPS) {
        ...
        // ใช้ 0.99 แทน 0.95
        const p99Lat = latencies[Math.floor(0.99 * latencies.length)] || 0;
...
async function main() {
    ...
    console.log('⏳ กำลังเตรียมข้อมูล Policy 100,000 ชุดลง Redis...');
    ...
    try {
        ...
        finalReport.push(...(await runIncrementalWorkload("4. Proposed MLVM", async (i) => {
            ...
            if (data) {
                ...
                if (policy.level === 'L2') { /* Logic */ }
            }
        })));

        // --- บันทึก CSV (ปรับหัวข้อเป็น P99) ---
        ...
        await fs.writeFile('final_p99_report.csv', header + rows, 'utf8');
        console.log(`\n🎉 บันทึกไฟล์สำเร็จ: final_p99_report.csv (คำนวณด้วย P99 เรียบร้อย)`);
